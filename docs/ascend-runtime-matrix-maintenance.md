...
当前源码政策：`vllm-hust` 与 `vllm-ascend-hust` 是从官方上游直接重建的组织仓库。官方上游分别为 `vllm-project/vllm` 与 `vllm-project/vllm-ascend`；重建前 commit 和旧 fork-only 历史不得作为当前源码或当前验证依据。稳定 release 镜像、组织 main 快照和 plugin Docker 开发候选必须分别建 `source_profiles`，不得因同属官方上游谱系就互相继承验证状态。

## 新版本更新步骤

1. 只从 vLLM Ascend GitHub release、版本化官方文档、release commit 的 Dockerfile、Quay Registry V2 API 和华为 Ascend 官方文档取数。
2. 枚举正式版的 A2/A3/A5/310P 与 Ubuntu/openEuler tag。main、nightly、dev、模型专用和镜像站独有 tag 不进入批准基线；官方 nightly 仅进入 `discovery_only.nightly_snapshots`。
...
5. 解析上游 core/plugin release tag 的完整 40 位 commit。另记录两个重建后组织仓库 HEAD、其功能性上游父提交和观察时的上游 HEAD；禁止只写 `main`。没有 OCI revision/provenance 时，明确标注为 release 引用 commit，而非镜像构建证明。
...
11. 对 nightly 同时处理单平台 manifest 与多平台 image index，记录 tag 当时的 reference digest、平台集合、ARM64 manifest/config digest、config 创建时间和 history 中可复核的 vLLM ref。tag 漂移、OS 构建版本不一致或缺 ARM64 都必须显式告警，不能自动更新 digest。
12. 若 core 的构建隔离 Torch 与 plugin 运行时 Torch 不同，分别记录两个约束，并以 `VLLM_TARGET_DEVICE=empty` 的实际 overlay 构建结果和真实 NPU 运行结果裁决；不得只比较版本字符串后宣称兼容或冲突。
...
- nightly tag 变化通常是预期事件，但仍需人工复盘架构、OS、vLLM ref 和依赖栈后创建新快照；旧快照 digest 保留，不原地静默覆盖。
...
- 每周：运行本地结构校验和 Registry digest 校验；稳定 tag 变化按供应链事件处理，nightly 变化按人工复盘流程处理。
