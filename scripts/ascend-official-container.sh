...
CONTAINER_NPU_DEVICES="${CONTAINER_NPU_DEVICES:-${VLLM_HUST_ASCEND_CONTAINER_NPU_DEVICES:-}}"
CONTAINER_PRIVILEGED="${CONTAINER_PRIVILEGED:-${VLLM_HUST_ASCEND_CONTAINER_PRIVILEGED:-1}}"
CONTAINER_REQUIRE_EXCLUSIVE_NPU_DEVICES="${CONTAINER_REQUIRE_EXCLUSIVE_NPU_DEVICES:-${VLLM_HUST_ASCEND_CONTAINER_REQUIRE_EXCLUSIVE_NPU_DEVICES:-0}}"
...
main() {
  ...
  if [[ -n "$CONTAINER_NPU_DEVICES" ]]; then
    manager_cmd+=(--npu-devices "$CONTAINER_NPU_DEVICES")
  fi

  if [[ "$CONTAINER_PRIVILEGED" == "0" || "$CONTAINER_PRIVILEGED" == "false" ]]; then
    manager_cmd+=(--no-privileged)
  fi

  if [[ "$CONTAINER_REQUIRE_EXCLUSIVE_NPU_DEVICES" == "1" ]]; then
    manager_cmd+=(--require-exclusive-npu-devices)
  fi
