...
def canary_spec(target, *, running: bool) -> DeploymentSpec:
    ...
    rendered = {"instance_id": CANARY_ID, "policy_sha256": target.policy_sha256,
                "running": running}
    value = {
        "schema": "vllm-hust.deployment-spec/v1",
        "image": {"id": "sha256:" + target.policy_sha256,
        ...
        "mods": ([{"id": CANARY_COMPONENT, "artifact": artifact,
                   "manifest": {"schema_version": "0.2-experimental",
                                "extension_id": CANARY_COMPONENT}}] if running else []),
        "model": {"id": "dev-hub/inert-cpu-canary", "revision": target.policy_sha256[:40],
                  "path": target.cwd, "files_sha256": target.policy_sha256},
        ...
        "resources": {"devices": [0], "tp": 1, "pp": 1,
                      "graph": {"mode": "graph", "configuration": {"capture_sizes": [1]}},
                      "ports": [], "mounts": []},
        "launch": {"interpreter": target.argv[0], "argv": list(target.argv),
                   "environment": dict(target.environment), "working_directory": target.cwd,
                   "plugin_allowlist": [CANARY_COMPONENT] if running else [],
                   "resolved_options": {"tensor_parallel_size": 1, "pipeline_parallel_size": 1,
                                        "model": "dev-hub/inert-cpu-canary", "enforce_eager": False,
                                        "compilation_config": {"capture_sizes": [1]}}},
        "provider": {"id": "inert-canary", "source_sha": target.policy_sha256[:40],
                     "configuration": {"cpu_only": True}, "rendered": rendered,
                     "rendered_sha256": digest(rendered),
                     "qualification": {"receipt_sha256": target.policy_sha256,
                                       "status": "qualified"}},
        "secrets": [],
    }
...
class CanaryBackend:
    ...
    def qualify(self, registration, spec):
        return (registration == {
            "instance_id": CANARY_ID, "owner_id": "host-broker",
            "profile_id": "inert-cpu-self-test", "backend_id": "inert-canary",
    ...
    def inspect(self, registration):
        ...
        if running:
            identity = {"boot_id": Path("/proc/sys/kernel/random/boot_id").read_text().strip(),
                        ...
                        "processes": [{"pid": resource["pid"], "start_ticks": resource["start_ticks"],
                                       "role": "inert-canary", "rank": None}]}
            spec = self.running
            components = [CANARY_COMPONENT]
        else:
            ticks = _process_start_ticks(1)
            suffix = "initial" if resource is None else str(resource.get("stop_operation_id", "stopped"))
            identity = {"boot_id": Path("/proc/sys/kernel/random/boot_id").read_text().strip(),
                        ...
                        "processes": [{"pid": 1, "start_ticks": ticks,
                                       "role": "host-boot-witness", "rank": None}]}
            spec = self.stopped
            components = []
        return {"instance_id": CANARY_ID, "spec_hash": spec.sha256,
                "identity": identity, "captured_at": self.clock(), "healthy": True,
                "components_executed": components, "inference_verified": True}
...
class CanaryCoordinator:
    ...
    def __init__(self, store, target, adapter, authority, *, clock=time.time):
        ...
        self.controller = Controller(store, {"inert-canary": self.backend},
                                     admin_uids=[os.geteuid()], enabled=True, clock=clock)
        self._ensure_registered()

    def _registration(self):
        return {"instance_id": CANARY_ID, "owner_id": "host-broker",
                "profile_id": "inert-cpu-self-test", "backend_id": "inert-canary",
                "actions": ["apply", "disable", "rollback"],
                "owner_uids": [os.geteuid()],
                "fencing_receipt_sha256": self.target.policy_sha256}
    ...
    def _ensure_registered(self):
        try:
            ...
            if str(exc) != "not_found":
                raise
            require(self.adapter.inspect(self.target)["state"] == "stopped",
    ...
    def status(self):
        ...
        return {**described, "generation": instance["generation"],
                "controllerStatus": instance["status"],
                "operationId": instance["operation"], "effective": False}

    def run(self, action: str):
        require(action in self.target.actions and action in {"start", "stop"},
                "lifecycle_action_not_allowed")
        before = self.backend.refresh()
        require((action == "start" and before["state"] == "stopped") or
                (action == "stop" and before["state"] == "running"),
                "canary_state_conflict")
        candidate = self.backend.running if action == "start" else self.backend.stopped
        deployment_action = "apply" if action == "start" else "disable"
        plan = self.controller.plan(CANARY_ID, deployment_action, candidate.value(),
                                    ttl=60, verify_seconds=30, recovery_seconds=120)
        approval = self.controller.approve(plan["plan_id"])
        operation = self.controller.begin(plan["plan_id"], approval)
        inverse = "stop" if action == "start" else "start"
        forward_grant = self.authority.mint(operation, self.target.command_sha256(action),
                                            os.geteuid(), ttl=30)
        peer = self._owner_peer()

        def execute(grant, lifecycle_action):
            lease = self.authority.claim(grant, peer,
                                         self.target.command_sha256(lifecycle_action))
            try:
                if lifecycle_action == "start":
                    self.adapter.start(self.target, operation, lease, peer, self.authority)
                else:
                    self.adapter.stop(self.target, operation, lease, peer, self.authority)
        ...
        def restore():
            restore_grant = self.authority.mint(
                operation, self.target.command_sha256(inverse), os.geteuid(), ttl=60)
            execute(restore_grant, inverse)

        result = self.controller.execute_external(
            operation,
            lambda: execute(forward_grant, action),
            restore,
        )
        require(result["phase"] == "committed", "canary_operation_not_committed")
        try:
            self.authority.claim(forward_grant, peer, self.target.command_sha256(action))
